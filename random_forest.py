...
class RandomForest:
    ...
    def build(self, X, y):
        ...
        for tree_i in range(self.num_trees):
            ...
            random_XY_indicies = self.rng.permutation(N)
            random_D_indicies = self.rng.permutation(D)
            data_ids = self.rng.choice(random_XY_indicies, num_data_per_tree, replace=True)
            feat_ids = self.rng.choice(random_D_indicies, num_features_per_tree, replace=True)
            X_sub = X[tuple(data_ids),:][:, tuple(feat_ids)]
            y_sub = y[tuple(data_ids),:]
            # ====================================================
